...
function User({user, setUser, scores}) {
    ...
    function getLead() {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            lead.push(
                <div key={i} className="w-full pl-10 pr-10 mt-10 text-white font-bold">
    ...
    return(
        ...
        {showLead && (
            <div className="block md:hidden fixed h-full w-full top-0 left-0 bg-black/50 z-100">
                ...
                <button onClick={() => {setShowLead(false)}} className="block md:hidden fixed h-10 w-10 top-[5%] left-[66%] bg-[#242424] text-white border-4 border-black text-xl font-bold hover:translate-x-1 transition-transform duration-500 ease-in-out">≡</button>
            </div>
        )}
        <button onClick={() => {setShowLead(true)}} className="block md:hidden fixed h-10 w-10 top-[10%] left-[5%] bg-[#242424] text-white border-4 border-black text-xl font-bold hover:translate-x-1 transition-transform duration-500 ease-in-out">≡</button>
        <div className="flex flex-col fixed w-[80%] md:w-[50%] h-[80%] top-[10%] right-[5%] bg-[#242424] border-5 border-black items-center">
            ...
            <input type="text" value={tempUser} maxLength={25} onChange={(e) => {setTempUser(e.target.value)}} className="bg-white border-5 border-black text-xl font-bold pl-5 h-15 w-100 mt-5"/>
